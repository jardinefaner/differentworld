...
class OddOneOutScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return DataSeededGame(
      ...
      seed: (cards) {
        if (cards.isEmpty) {
          return {'rounds': const <Map<String, dynamic>>[], 'i': 0, 'r': false, 'd': false};
        }
        // Up to 8 distinct rounds. Spread the seeds so consecutive rounds pick
        // different categories; seed by deck size so present + control derive
        // the same set from the same wire-state (built once, broadcast).
