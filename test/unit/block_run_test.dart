...
void main() {
  group('buildBlockRun', () {
    ...
    test(
      'a closed block becomes the close beat; everything else is activity',
      () {
        final beats = buildBlockRun([
          _blk(
            title: 'Make',
    ...
    test(
      'a session slug flags a runnable session; none leaves lines empty',
      () {
        final beats = buildBlockRun([
    ...
    test(
      'buildBlockRunAligned aligns beats with source blocks (drops + sorts)',
      () {
        final inputs = [
          _blk(
            id: 'late',
    ...
    test(
      'liveBlockOrder is deterministic for equal start times (blockId tiebreak)',
      () {
        final inputs = [
          _blk(id: 'z', title: 'Z'),
          _blk(id: 'a', title: 'A'),
    ...
    test(
      'routine blocks get a run-script (steps + tools), not just a title',
      () {
        final arrival = buildBlockRun([
          _blk(title: 'Arrival & check-in'),
        ]).single;
        ...
        final pickup = buildBlockRun([
          _blk(title: 'Pickup', kind: 'closed'),
        ]).single;
