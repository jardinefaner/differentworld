...
void main() {
  group('ContentBankDao — staff picture CRUD + offline swap', () {
    ...
    test('updatePicturePath swaps the image, preserves the label', () async {
      ...
      await db.contentBankDao.updatePicturePath(
        'pic1',
        'sp1/custom_picture/pic1/x.jpg',
      );
      final rows = await db.contentBankDao
          .watchOwnByKind('sp1', 'picture')
          .first;
    ...
    test(
      'watchOwnByKind is scoped to space + kind (excludes global/other)',
      () async {
        await db.contentBankDao.createStaffItem(
          id: 'a',
          spaceId: 'sp1',
