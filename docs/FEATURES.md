...
- `day_run.dart` — `DayBeat`, `DayBeatKind` enum (open/question/verbs/rule/watch/play/name/bridge/ask/activity/close). Pure data; `buildDayRun(world, weekLog, games)` assembles the ordered slide sequence for the room.
- `day_run_screen.dart` (`DayRunScreen`) — `/play-today`: immersive run of show. Full-screen `PageController` slide-by-slide; enters `SystemUiMode.immersiveSticky` + `castImmersiveProvider` on mount; restores on dispose. Teacher just taps forward; no nav hunting. Reached via "Play today" hero button on This Week screen. `page.play-today` omnibox entry → `/play-today`.
**Status**: This Week hub + world projection (Cast) + worksheets + Today card shipped (waves "week engine" + "worksheets" + "cast", commit range fbfdda3–206b108). Book screen + curriculum importer shipped (waves "the Book" + "import curriculum activities", commits 2021813 + 5d4db52). Play Today (DayRunScreen) + Big Thinking system games + thinking_screen shipped (waves feat/kid-book + feat/curriculum-activity-import, commit 1320864). Activity matcher verb-tagging library hookup, spell timers still deferred.
...
- *Print toolkit screen* — `lib/features/toolkit/print_toolkit_screen.dart`. `/print`: a menu of PDF generators — "Verb cards" (12 full-page cards), "Timer spell cards" (FREEZE/MOVE/CREATE/SHARE/WONDER), "Verb→job reference" (which job each verb pick becomes), "Gesture guide", "Reference card". Each tile triggers the corresponding function from `toolkit_pdf.dart` via the OS print sheet. Word-forward (Helvetica; emoji sticker on after laminating). NOTE: the per-child Summer Book is NOT here — it lives on each kid's Book screen.
- *Toolkit PDF* — `lib/features/toolkit/toolkit_pdf.dart`. PDF generation functions: `printVerbCards`, `printTimerSpellCards`, `printVerbJobReference`, and others. Built-in Helvetica throughout — offline-safe, no `PdfGoogleFonts` call.
**Depends on**: Action Words (`verb_roles.dart` imported by `print_toolkit_screen.dart` for the verb→job reference).
**Consumed by**: Nothing.
**Last verified**: 2026-06-07
