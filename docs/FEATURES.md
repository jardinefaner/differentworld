...
- *SpeakVoices* — `lib/features/speak/speak_voices.dart`. The `SpeakVoice` catalog (ElevenLabs voice_id + label) the picker offers — Mark / Hope / Elise / Dan; first is the default. IDs are NOT secret (the API key is); add/swap by editing the list, no server change. The chosen id rides to the Edge Function, which caches audio per (voice, text).
