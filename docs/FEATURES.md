...
- *Compose-to-draft* ("omnibox composes") — `lib/features/omnibox/compose_intent.dart` (pure, rules-based parser: kind + when + title, no LLM) + `compose_draft_seed.dart` (a `Notifier` courier). When a capture-mode phrase reads as a schedule intent ("field trip to the pond Friday", "art tomorrow at 2", "closed Friday"), a `_ComposeDraftCard` appears ABOVE the note fallback showing the parsed structure as chips; tapping (or Return) drafts a block into `BlockEditScreen` pre-filled with kind + note at the parsed day/time, scheduled into the first cohort — nothing is saved until the teacher confirms in the editor. Additive: the note fallback (`_CaptureHeroCard`, demoted to a `muted` variant) never goes away, so a false positive costs only a glance. Parser covered by `test/unit/compose_intent_test.dart` (16 cases: fires on intents, stays null for notes-to-self, weekday/time resolution).
