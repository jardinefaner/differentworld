...
**Consumed by**: Photo turns (`EveryoneOnce`); every game via the control bar and the four standalone activity screens via `OursFooter` (Room tools); Penny and Potions (Keep this). `GridGame` still keeps its own `alternates` API and the picker its own screen — those remain deliberately un-rewired.
...
**Surfaces**: the per-game screens under `games/`, `game_scaffold.dart` / `game_stage.dart` (the shared shell + raw stage), `game_registry.dart` (add a game in ONE place).
