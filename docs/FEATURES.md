...
- `this_week_screen.dart` — `ThisWeekScreen` at `/this-week`: the live curriculum hub. Shows the active world (hero card: week number, name, question); "Cast to the room" button → `/present-world/:id`; "Activities" → `/action-words/activities?verbs=…`; "Worksheets" → `printWorldWorksheets`; "Explore all worlds" → `/action-words/different-worlds`. Director "Manage journey" tuning action opens a glass sheet to set / jump / clear the journey start date. Embeds `_FortnightSection` — the block's ten authored days (`JourneyDayRow` list, today badged; tapping each opens `showJourneyDaySheet`; renders nothing until the journey is active + blocks have loaded).
...
- `day_run.dart` — `DayBeat`, `DayBeatKind` enum (open/question/verbs/rule/watch/play/name/bridge/ask/activity/close). Pure data; `buildDayRun(world, weekLog, games)` assembles the ordered slide sequence for the room. Also exports `buildActivityArc(String activity)` — pure function that produces the 5-beat play→name→bridge→ask→close arc for any single activity; used by `ActivityArcScreen`.
...
- `action_words_kid_screen.dart` (`ActionWordsKidScreen`) — `/action-words/pick/:subjectId`: kid-facing morning pick. Device is handed to ONE child; they tap 3 of 12 verbs and confirm. Enters kid mode on mount via `kidModeProvider.notifier.enter()` + pins the route with `kidModeLockedRouteProvider`; staff exit is 5 taps in the top-left corner within 800 ms each, opening `showKidModeExitDialog`. On save calls `ActionWordsActions.setPicks` (same path as teacher pick — no new table). Post-save shows a `_Celebration` screen with the three chosen verbs; child shows a grown-up. Launched from the "Let {name} pick" `IconButton` trailing on each `_KidRow` in `ActionWordsScreen`.
- `growth_arc.dart` — `buildGrowthArc(firstName:, collection:)` pure builder. Compiles an `ActionWordsCollection` into an ordered `List<DayBeat>`: an opening beat (name + day count), top-4 verb totals, up-to-5 most-visited worlds, world-count beat, emerging title, closing beat. Empty-collection path produces a two-beat "story starts" arc. Deterministic, unit-testable, zero network.
- `growth_arc_screen.dart` (`GrowthArcScreen`) — `/growth/:subjectId`: the child's growth arc cast on the shared present spine (`BeatPresenter`). Reads `actionWordsCollectionProvider(subjectId)` + `subjectByIdProvider(subjectId)`; calls `buildGrowthArc`; delegates all presentation to `BeatPresenter` (accent `0xFF7C4DFF`, emoji ✨). Loading state shows a "Gathering the story…" text + Close button on a black scaffold. Launched from the "Play their story" `FilledButton` in `_Links` at the bottom of `CharacterSheetScreen`. Cast to the room at closing ceremony or sent home to family at pickup; no staff-PIN gate (this is a cast surface, not a kid-operated surface).
...
**Surfaces**:
- *Brain Breaks deck* — `lib/features/activity_runtime/brain_breaks_screen.dart`. 16 cards on mobile (Android/iOS), 15 off-mobile (Photo Studio hidden via `isMobileCapturePlatform`) + a Surprise button; each card pushes its activity route.
...
**Depends on**: Subjects (PostgREST), Guardians, Messages (offline-first via `by_guardian`), Settings (shared text-size sheet), Exports (received-reports card reads `myReceivedExportsProvider`), Incidents (`Incident` model + `FamilyIncidentCard` imported for the surfaced-incidents section).
