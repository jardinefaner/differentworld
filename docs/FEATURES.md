...
- `journey_day_sheet.dart` — `showJourneyDaySheet(context, day:, journeyDay:, block:, wallQuestion:, isToday:)` shared glass sheet (day number + block name header; focus text; "On the wall" question card; room/soundtrack/key-moment section; "Run today" FilledButton when `isToday: true` → pushes `/play-today`). Also exports `JourneyDayRow` — a tappable list tile (day number + title; today badged) used by `_FortnightSection` on `/this-week`.
