...
**Discovery surfaces**:
- Routes: `/action-words` (the morning pick roster), `/action-words/send` (Send home — end-of-day copy-to-clipboard parent messages), `/action-words/worlds` (Our worlds — the class's invented-world book), `/action-words/this-week` (This week's world — themed-world reveal + sensory "Become it" beats + world picker), `/action-words/activities` (Activity matcher — filter activities by verb), `/action-words/:subjectId` (the collection — worlds grid + verb bars + emerging title)
- Omnibox: `page.action-words` — "Action Words" (keywords: verbs, words, worlds, pick, reveal, collection, animal; morning contextTag). `page.this-weeks-world` — "This week's world" (keywords: world, this week, theme, themed, wildlife, safari, travel, water world, space, weekly world). No omnibox entries for `/send`, `/worlds`, or `/activities` — reached via chrome action buttons or in-context links.
- Slash: none
- Drawer/Rail: yes — "Action Words" nav destination, gated `onlyFor: canObserve`
- Settings: none (no opt-in cap yet)
**Capabilities**: Pick / run: `can_observe` (the daily-staff gate). Setting the week's world: `SpaceCaps.currentWorld` (`'current_world'`) string cap on the space, written via `spaceCapActionsProvider.setStringCap`. No migration — rides the existing `spaces.capabilities` JSONB.
**Data**: Reuses [entries](SCHEMA.md#entries) `kind='action_words'` — one row per (subject, date); `details` = `{verb_picks:[3], done:[…], note, word_of_day, world_name?}`. The revealed world is DERIVED from `verb_picks` (deterministic lookup), not stored. The week's themed world is stored in [spaces](SCHEMA.md#spaces) `capabilities.current_world`. NO new table.
**Surfaces**:
...
- `action_words_providers.dart` — `ActionWordsDay` (typed per-day), `ActionWordsCollection` (lifetime worlds + practiced-verb totals + emerging title), `ActionWordsActions` (setPicks/toggleDone/setNote/setWordOfDay/setWorldName — optimistic, find-or-create per day in a txn). `inventedWorldsProvider` (derived from the collection: worlds where `world_name` was user-authored). `currentThemedWorldProvider` — resolves the week's `ThemedWorld` from `currentSpaceProvider.caps.getString(SpaceCaps.currentWorld)`.
- `action_words_screen.dart` — the pick roster + the per-kid pick sheet (12-verb grid → live world preview → save). Top chrome carries two icon buttons: "Our worlds" → `/action-words/worlds` and "Send home" → `/action-words/send`. Embeds `_ThisWeekBanner` at the top of the child list — a tappable banner showing the current themed world (or an invitation to choose one); tapping pushes `/action-words/this-week`.
- `themed_world_screen.dart` — `/action-words/this-week`: shows the current themed world (emoji + name + room + blurb), the sensory "Become it" SenseBeat rows, a "Activities for this world" button → `/action-words/activities`, and a "Change this week's world" button that opens a `showGlassSheet` picker over all 6 starter worlds. Empty state when no world is set.
...
**Status**: building — core 4-step loop shipped (waves 1–4); Send home + world book shipped (wave 5); themed world of the week shipped (wave 6 — this commit). Next: activity matcher verb-tagging library hookup, today dashboard (word-of-day + kid cards), spell timers.
**Depends on**: Entries, Subjects, Viewer, Spaces (`currentSpaceProvider` for the `current_world` cap read/write).
