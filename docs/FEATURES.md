...
**Discovery surfaces**:
- Routes: `/breaks` (deck), `/activity/do-it` (Do It — `?group=` scopes the room record), `/activity/math` (Many Paths), `/activity/math-game` (Math Game), `/activity/photo` (Photo Studio — route exists on all platforms; screen degrades to "No camera here" off-mobile), `/activity/this-or-that` (Quick Picks), `/activity/starts-with` (Beat the Letter), `/activity/as-if` (Act It Out), `/activity/roles` (Role Cards), `/activity/pattern` (Make a Pattern), `/activity/discussions` (Group Discussions), `/activity/riddles` (Riddles), `/activity/breathe` (Mindful Minute), `/activity/fact-or-fib` (Fact or Fib), `/activity/story` (Story Starters), `/activity/rhyme-time` (Rhyme Time), `/activity/fill-blank` (Fill in the Blank), `/activity/letters` (Letters — `?group=` selects a cohort to pair), `/activity/penny` (Penny for a Thought), `/activity/potions` (Potions), `/activity/grid-reveal` (Reveal the Picture), `/activity/charades` (Charades on ONE device — the phone is the actor's card; the two-device session stays at `/live/charades`, hosted by Games/LiveSession)
- Omnibox: **every card in the library** — `lib/features/omnibox/deck_entries.dart` generates one `page.deck.<route>` entry per `DeckCard` from the same `breakDeck()` + `presentDeck` lists the library renders (title, tagline, the words people actually type — "would you rather", "simon says", "mad libs"), skipping the routes the catalog lists by hand (Do It, the picture games, the instruments). `test/unit/deck_omnibox_test.dart` asserts every card is searchable exactly once. Before 2026-09-14 only Do It was.
...
**Surfaces**:
- *Do-together library* — `lib/features/activity_runtime/brain_breaks_screen.dart` (`/breaks`, "Do together" in the drawer). The card LIST lives in `activity_deck.dart` as `breakDeck(camera:)` (40 cards on mobile, 39 off-mobile — Photo Studio follows `isMobileCapturePlatform`) + `presentDeck` (10 room-screen cards), so the omnibox reads the same list. Grouped by lane (Quick resets · On the room's screen · The day's rhythm) + a Surprise button; each card pushes its route; hold / right-click puts it on the room's screen. **Do It leads the deck.** The toggle-gated Heroes / Routines / Daily / Calm / Spellbook cards slot in after Do It when their settings switch is on.
...
- *Charades (deck launcher)* — the deck card pushes `/activity/charades` (`GameRunner(def: CharadesGame())`): on one device the scaffold renders the SECRET stage, so the phone in the host's hand is the actor's card and the room guesses; fullscreen and a cast TV render the room stage (category + count). `/live/charades` (two devices) is one long-press away. The deck card is unconditional (not platform-gated).
...
- *Room tools* — `lib/features/facilitation/room_tools.dart`. Facet 5 (what the adult can change, right now): one control, available from inside ANY running activity, opening Spotlight / Timer / Signals / Now & Next / Quick Poll over the top and returning you to where you were. The instruments all existed and none was reachable from inside an activity — a counselor mid-Bingo had to leave, walk back to the deck, find the card and lose their place.
- *Keep this* — `lib/features/facilitation/keep_this.dart`. Facet 6 (what to remember): ends an activity by keeping one thing as an `EntryKind.classMemory`. Keeps the PROMPT, not a transcript — what a room says is spoken aloud and the app never had it. Hides itself when there is no live block, because a class memory needs a group.
...
- *Run-scripts* — `room_beat.dart` (`RoomBeat` / `RunScript`), `run_script_view.dart` (one beat at a time, Next / Start on the floor), `run_script_wire.dart` (the cursor in the WIRE: brief on open, `next`/`back` walk the beats, every other intent is swallowed while briefing, Play again does NOT re-brief, `reveal + {rules: true}` re-opens the rules over a live board), `activity_run_scripts.dart` (the beats for the non-game activities) + `activity_brief.dart` (the local-cursor gate for mirror-only activity screens). 34 of 41 registry games and 9 of 11 activities brief the room; the exceptions are ledgered with reasons in `test/unit/run_script_test.dart`.
**Consumed by**: Photo turns (`EveryoneOnce`); every game via the scaffold (the briefing, Room tools in the pill or the panel, Keep this on the wrap beat) and the activity screens via `ActivityBrief` + `OursFooter`; Penny and Potions (Keep this). `GridGame` still keeps its own `alternates` API and the picker its own screen — those remain deliberately un-rewired.
...
- *Shape renderer* — `lib/features/live_session/shape_stage_view.dart` + `stage_shape.dart`. The ONE renderer for every described board (`ShapeKind.grid`), on the phone and the TV: styles (tiles · pads · holes · lattice), a colour slot per cell (identity — the receiver picks the colour), a progress bar, and per-cell motion (deal-in, flip, pop, shake, drop). New wire keys (`st`, `p`, `pr`) are ignored by older receivers, which draw tiles as before.
- *Do-together deck card* — `lib/features/activity_runtime/activity_deck.dart`. "Charades" card → `/activity/charades` (one device); `/live/charades` is the two-device session, one long-press away via `showCastToRoom`.
...
**Consumed by**: Today (`live_session_banner.dart` mounted in `today_sections.dart`; `activeSessionsProvider` drives the banner); LiveBoard (imports `CastSession` + `generateSessionCode` from this feature's cast spine); Games (every `GridGame` draws through `ShapeStageView`; the cast receiver wraps a stage in `GameMotion` + `CelebrationLayer`).
**Last verified**: 2026-09-14
...
- *Activity library card* — `DeckCard('Live Board', lane: DeckLane.screen, route: '/live-board')` in `lib/features/activity_runtime/activity_deck.dart`. Sits in the library's "On the room's screen" lane; the primary visual entry point for the feature.
...
**Purpose**: The host-run game engine — one registry (`game_registry.dart`) resolves a game id to its `GameDefinition`, so a cast session, a join-by-code and the launcher all render the same game without navigating to its route first. 41 registry games: the prompt games, the deck-seeded picture-card games (docs/CARD_GAMES.md) and the nineteen classics on `GridGame`. **Every one briefs, plays, ends on screen and restarts** (2026-09-14): 34 carry a room-facing run-script; the seven that don't are instruments, ledgered with reasons in `test/unit/run_script_test.dart`.
**Personas served**: Coach Sam, Brianna, Ava (the room-facing moments); Pat — a substitute can open any card and run it from the screen's own beats.
**Discovery surfaces**:
- Routes: `/activity/<id>` (solo) and `/live/<id>` (cast) per game; `/present/<id>` for the room-screen presentables; `/present` and `/breaks` (the one library)
- Omnibox: yes — every game, generated from the deck (`omnibox/deck_entries.dart`)
- Drawer: yes — "Do together" (`/breaks`)
- Settings: yes — Preferences → **Game motion** (`GameMotionTile`, default on): the deal-in, flips, drops, glow, the celebration burst and the haptic taps; the OS reduce-motion setting wins underneath it.
**Capabilities**: staff; kid-facing surfaces run in kid mode.
**Data**: [content_items](SCHEMA.md#content_items) (the content bank), plus per-game local state. Motion preference in SharedPreferences (`settings.game_motion`).
**Surfaces**:
- *The scaffold* — `game_scaffold.dart`: the briefing first (`RunScriptView` from the wire cursor), then the stage; a stage-owning game (`buildLiveStage`) gets the **RoundWrap** (`round_wrap.dart`) under its board when the round ends — the closing line (`GameDefinition.outcomeLine`), Play again · Done · How to play, the add-ours door and the keepsake — and Room tools + Start over + How to play in the top pill. On ONE device a game with a secret (Charades) shows `buildSecretStage`. Every stage sits in a `CelebrationLayer`.
- *The celebration* — `celebration.dart`: a burst in the game's accent, warm paper and gold on the done edge, plus a heavy haptic on the phone (never on a room screen). Framework-level: the eleventh game celebrates like the first.
- *Motion* — `game_motion.dart`: `GameMotion` scope (enabled / haptics) read by the renderer and the burst; `gameMotionProvider` + `GameMotionTile`.
- *The board renderer* — `lib/features/live_session/shape_stage_view.dart` (LiveSession owns the file; the classics all draw through it): styles (tiles · pads · holes · lattice), a colour slot per cell, a progress bar, and an animated cell that reads what changed — deal-in, flip, pop, shake, drop. Every cell takes a tap; the game's rule declines.
- *The classics* — `grid_game.dart` + `games/*`: a board and a rule each. `style`, `progressFor`, `present` (what the room may see), `onFlag` (hold — Minesweeper), `outcomeFor` (the ending), `howToPlay` (the beats). Deck-seeded ones (Bingo, Guess Who, Spot the Difference) seed through `classic_boards_screen.dart`, which goes through the game's `initialTally` + labels — the seed path IS the app path.
- *Run-scripts* — `RunScriptWire` (Facilitation): brief on OPEN; Play again deals straight away; the rules re-open from the wrap beat or the pill via `reveal + {rules: true}`, over the wire so a paired TV re-briefs too.
- `game_stage.dart` (the raw stage atoms), `game_registry.dart` (add a game in ONE place), `game_fullscreen.dart` (present on the big screen).
**Depends on**: GameContent, LiveSession (casting, the renderer), ActivityRuntime, Facilitation (run-scripts, Room tools, Keep this).
**Consumed by**: [LiveSession](FEATURES.md#livesession) (the cast receiver resolves games through the registry and draws them with the same renderer + celebration).
**Last verified**: 2026-09-14
