...
- *Speak screen* — `lib/features/speak/speak_screen.dart`. Paste prompt → pick a voice + type + mode → "Speak it" → full-bleed immersive performance (`/speak` is an immersive route, so AppShell hides the omnibox bar). Perform chrome: mode menu / type toggle / New text / Replay. A **transport bar** (`_TransportBar`) at the bottom: play/pause, a drag-to-seek scrubber, elapsed/total time. Tap the stage to pause/resume; a finished read dims + shows a replay glyph. A **Recent** list on the input screen replays past pieces with no re-synthesis (see Speak history). `_SpeakStageHost` drives the active mode from a per-frame ticker (idle when paused) + owns playback; loading is an inline "Voicing…" spinner; failure shows a clear "not set up / offline" live-region note.
- *Speak history* — `lib/features/speak/speak_history.dart` + `SpokenScript` JSON. Local (SharedPreferences) recents — text + voice + the timed script — so a tap replays from the stored timings + the already-cached audio: no Edge Function, no ElevenLabs. Newest-first, deduped, capped at 20; "Clear" wipes it. Also serves "go back to where I was" (persists across navigation).
