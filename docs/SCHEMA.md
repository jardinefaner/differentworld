...
- `kind` (text — `observation` / `meal` / `nap` / `diaper` / `incident` / `departure` / `action_words` / `skill_measure` / `work_sample` / `reflection` / `world_rule` / `did_it` / `hero` / `daily_response` / `recap` / etc.)
